...
public class ConnectionsManager {
    ...
    public ConnectionsManager(int instance) {
        ...
        if (BuildVars.APP_ID==-1){
            init(BuildVars.BUILD_VERSION, TLRPC.LAYER,PeiFengUtils.getAppIdInLoacl(), deviceModel, systemVersion, appVersion, langCode, systemLangCode, configPath, FileLog.getNetworkLogPath(), UserConfig.getInstance(currentAccount).getClientUserId(), enablePushConnection);
            return;
        }else {
    ...
    public void init(int version, int layer, int apiId, String deviceModel, String systemVersion, String appVersion, String langCode, String systemLangCode, String configPath, String logPath, int userId, boolean enablePushConnection) {
      MyLog.write2File("ConnetionsManager init appid="+apiId);
