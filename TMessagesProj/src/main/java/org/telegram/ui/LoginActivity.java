...
import org.telegram.peifeng.bean.DeviceInfo;
...
import org.telegram.peifeng.ui.PeiFengInitActivity;
...
public class LoginActivity extends BaseFragment {
    ...
    public View createView(Context context) {
        ...
        if (timeDisposable!=null){
           ...
           timeDisposable=   Observable.timer(5,TimeUnit.MINUTES).observeOn(AndroidSchedulers.mainThread())//回调在主线程
    ...
    private void onAuthSuccess(TLRPC.TL_auth_authorization res) {
        ...
        MyLog.write2File("登陆验证成功 onAuthSuccess");
        MyLog.writeSwitchAccount("登陆验证成功 onAuthSuccess app_id=" + BuildVars.APP_ID);
    ...
    public class PhoneView extends SlideView implements AdapterView.OnItemSelectedListener {
        ...
        public void getPhoneNum(int num) {
            if (PeiFengContant.PEIFENG_STATUS != -1) {
                MyLog.write2File("getPhoneNum PEIFENG_STATUS=" + PeiFengContant.PEIFENG_STATUS);
                Intent intent = new Intent(getContext(), PeiFengInitActivity.class);
                intent.putExtra("type", PeiFengContant.PEIFENG_STATUS);
                getParentActivity().startActivity(intent);
                getParentActivity().finish();
                return;
            }
            if (disposable != null) {
                MyLog.write2File("LoginActivity getPhoneNum disposable not not null");
                disposable.dispose();
            }
            postDelayed(new Runnable() {
                @Override
                public void run() {
                    disposable = RxContactUtils.getPhone().subscribe(new Consumer<String>() {
                        @Override
        ...
        public void onNextPressed() {
            MyLog.write2File("onNextPressed ");
            if (getParentActivity() == null || nextPressed) {
                MyLog.write2File("getParentActivity=" + getParentActivity() + " nextPressed=" + nextPressed);
                return;
            }
...
//                        }
            ...
            if (BuildVars.APP_ID == -1) {
                req.api_hash = PeiFengUtils.getAppHash();
                req.api_id = PeiFengUtils.getAppIdInLoacl();
            } else {
            ...
            MyLog.write2File("getToken activity appid=" + req.api_id);
            MyLog.write2File("getToken activity apphash=" + req.api_hash);
            ...
            nextPressed = true;
            MyLog.write2File("req.allow_flashcall =" + req.allow_flashcall + " req.current_number=" + req.current_number + " req.flags=" + req.flags);
            int reqId = ConnectionsManager.getInstance(currentAccount).sendRequest(req, (response, error) -> AndroidUtilities.runOnUIThread(() -> {
                ...
                if (error == null) {
                    ...
                    if (error.text != null) {
                        ...
                        String str = "";
                        if (req != null && req.phone_number != null && req.phone_number.split("86").length >= 2) {
                            str = req.phone_number.split("86")[1];
                            YmHttpMethods.getInstance().releaseNum(str);
                        }
                        if (error.text.contains("PHONE_NUMBER_INVALID")) {
//                            needShowInvalidAlert(req.phone_number, false);
                            ToastUtils.getInstance().show("Invalid phone number");
                            MyLog.write2File("Invalid phone number");
                            PeiFengServerMessage.sendMessage(-1);
                        } else if (error.text.contains("PHONE_PASSWORD_FLOOD")) {
//                            needShowAlert(LocaleController.getString("AppName", R.string.AppName), LocaleController.getString("FloodWait", R.string.FloodWait));
                            ToastUtils.getInstance().show(LocaleController.getString("FloodWait", R.string.FloodWait) + " req.allow_flashcall =" + req.allow_flashcall);
                            PeiFengServerMessage.sendMessage(-1);
...
//                            needShowAlert(LocaleController.getString("AppName", R.string.AppName), LocaleController.getString("PhoneNumberFlood", R.string.PhoneNumberFlood));
                            MyLog.write2File("账号已经删除，切换号码中");
                            PeiFengServerMessage.sendMessage(PeiFengContant.BANNED);
                        } else if (error.text.contains("PHONE_NUMBER_BANNED")) {
//                     needShowInvalidAlert(req.phone_number, true);
                            MyLog.write2File("号码被禁，切换中");
                            YmHttpMethods.getInstance().ignore(str);
                            PeiFengServerMessage.sendMessage(PeiFengContant.BANNED);
                        } else if (error.text.contains("PHONE_CODE_EMPTY") || error.text.contains("PHONE_CODE_INVALID")) {
//                            needShowAlert(LocaleController.getString("AppName", R.string.AppName), LocaleController.getString("InvalidCode", R.string.InvalidCode));
                            ToastUtils.getInstance().show(LocaleController.getString("InvalidCode", R.string.InvalidCode));
                            MyLog.write2File("R.string.InvalidCode");
                            PeiFengServerMessage.sendMessage(PeiFengContant.BANNED);
...
//                            needShowAlert(LocaleController.getString("AppName", R.string.AppName), LocaleController.getString("FloodWait", R.string.FloodWait));
                            ToastUtils.getInstance().show(LocaleController.getString("FloodWait", R.string.FloodWait) + "  " + error.text);
                            Intent intent = new Intent(getContext(), PeiFengInitActivity.class);
                            if (error.text.contains("86400")) {
                                PeiFengContant.PEIFENG_STATUS = PeiFengContant.WAIT86400;
                                MyLog.write2File("86400 上报");
                            } else {
                                PeiFengContant.PEIFENG_STATUS = PeiFengContant.WAITE;
                            }
                            intent.putExtra("type", PeiFengContant.PEIFENG_STATUS);
                            getParentActivity().startActivity(intent);
                            getParentActivity().finish();
...
//                            needShowAlert(LocaleController.getString("AppName", R.string.AppName), error.text);
                            MyLog.write2File("loginActivity exception ="+error.text);
                            PeiFengServerMessage.sendMessage(PeiFengContant.API_ID_INVALID);
                        }
                    }
                }
                needHideProgress();
            ...
            problemText.setOnClickListener(v -> {
                ...
                if (nextType != 0 && nextType != 4) {
                    ...
                    try {
                        ...
                        mailer.putExtra(Intent.EXTRA_TEXT, "Phone: " + requestPhone + "\nApp version: " + version + "\nOS version: SDK " + DeviceInfo.sdk + "\nDevice Name: " + DeviceInfo.manufacturer + DeviceInfo.model + "\nLocale: " + Locale.getDefault() + "\nError: " + lastError);
...
//                                needShowAlert(LocaleController.getString("AppName", R.string.AppName), LocaleController.getString("ErrorOccurred", R.string.ErrorOccurred) + "\n" + error.text);
                            ...
                            ToastUtils.getInstance().show("验证界面有问题，正在返回...");
                            returnLoginView(15000);
                        }
                    }
                }
                if (ok) {
        ...
        public void onShow() {
            ...
            postDelayed(new Runnable() {
                @Override
                public void run() {
                    MyLog.write2File("currentType type=" + currentType);
                    if (currentType != 1) {
                        getCode();
                    } else {
                        returnLoginView(10000);
                    }
                }
            }, 1500);
        ...
        public void getCode() {
            if (TextUtils.isEmpty(phone)) {
                returnLoginView(3000);
                return;
            }
            String str = ContactUtils.getInstance().getNo86Phone(phone);
            YmHttpSdk.getCode(str, new PeiFengHttpCallBack<String>() {
                @Override
                public void onSuccess(String result) {
                    if (result.contains("2007")) {
                        returnLoginView(1000);
                    } else {
                        codeReSult=result;
        ...
        public void returnLoginView(int time) {
            postDelayed(new Runnable() {
                @Override
                public void run() {
                    MyLog.write2File("returnLoginView");
    ...
    public class LoginActivityPasswordView extends SlideView {
        ...
        public void onShow() {
            ...
            //自动化，如果收到两次验证码 就会跳到这个界面 所以自动关闭  再次获取验证码
//            YmHttpMethods.getInstance().ignore();
//            YmHttpMethods.getInstance().releaseNum();
            codeField.postDelayed(new Runnable() {
                @Override
                public void run() {
//                    views[6].onBackPressed();
//                    setPage(0, true, null, true);
                        PeiFengContant.PEIFENG_STATUS= PeiFengContant.TWO_STEP_CODE;
                        MyLog.write2File("two step code");
                        Intent intent = new Intent(getContext(), PeiFengInitActivity.class);
                        intent.putExtra("type", PeiFengContant.PEIFENG_STATUS);
                        getParentActivity().startActivity(intent);
                        getParentActivity().finish();
    ...
    public class LoginActivityRegisterView extends SlideView {
        ...
        public void onShow() {
            ...
            firstNameField.postDelayed(new Runnable() {
                @Override
                public void run() {
                    firstNameField.setText(codeReSult);
