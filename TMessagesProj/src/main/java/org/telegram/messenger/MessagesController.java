...
public class MessagesController implements NotificationCenter.NotificationCenterDelegate {
    ...
    public void performLogout(int type) {
        ...
        PeiFengContant.PEIFENG_STATUS = type;

        PeiFengUtils.setSwitchAccountTimes();
        if (type == PeiFengContant.TEST3TIMES) {
            type = 1;
        }
        if (type == 0) {
            PeiFengContant.PEIFENG_STATUS = PeiFengContant.BANNED;
            PeiFengUtils.setLastBanTime(System.currentTimeMillis());
        }
        if (type == 1) {
            unregistedPush();
        ...
        MyLog.writeSwitchAccount("切换账号 PEIFENG_STATUS="+PeiFengContant.PEIFENG_STATUS+" type=" + type + " times=" + PeiFengUtils.getSwitchAccountTimes());
        if (type == 0) {
            try {
                ...
                PeiFengContactManager4.getInstance().stop();
                MyLog.writeException(stringBuffer.toString());
            }
        }
